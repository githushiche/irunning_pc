...
public class StepDataDao {
	...
	public StepEntity getCurDataByDate(String curDate) {
		...
		Cursor cursor = stepDb
				.query("step", null, null, null, null, null, null);
		while (cursor.moveToNext()) {
			String date = cursor.getString(cursor
					.getColumnIndexOrThrow("curDate"));
			if (curDate.equals(date)) {
				String steps = cursor.getString(cursor
						.getColumnIndexOrThrow("totalSteps"));
	...
	public List<StepEntity> getAllDatas() {
		...
		while (cursor.moveToNext()) {
			...
			String totalSteps = cursor.getString(cursor
					.getColumnIndex("totalSteps"));
	...
	public void updateCurData(StepEntity stepEntity) {
		...
		values.put("curDate", stepEntity.getCurDate());
		values.put("totalSteps", stepEntity.getSteps());
		stepDb.update("step", values, "curDate=?",
				new String[] { stepEntity.getCurDate() });
